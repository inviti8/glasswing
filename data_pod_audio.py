...
def determine_image_type(img_hash: str, img_info: Dict[str, Any], 
                       app) -> str:
    """
    ...
    try:
        ...
        for img_hash in processed_hashes:
            try:
                ...
                print(f"📋 Image {img_info.get('name', 'Unknown')} detected as type: {image_type}")
                
                # Get metadata using existing function
                try:
                    metadata_list = await app.get_img_metadata(img_path)
                    metadata = metadata_list[0] if metadata_list and isinstance(metadata_list, list) else {}
        ...
        ninjs_data = {
            ...
            'creator_public_key': get_current_user_stellar_public_key(app),
...
async def process_data_pod_locally(
    ...
    try:
        ...
        print(f"🔐 Subscriber stellar secret (first 16): {subscriber_stellar_secret[:16]}...")
        ...
        print(f"🔐 Using debug secret + creator public key: {creator_public_key[:16]}...")
        ...
        if not all([download_ipfs_image, new_deciphered_img, recover_aposematic_img, image_to_base64_uri]):
            raise ValueError("Missing required image processing functions in main module")
        ...
        for item in data_pod.get('items', []):
            try:
                ...
                if not href:
                    print(f"❌ No href found in rendition for item: {item.get('title')}")
                    continue
                ...
                if image_type == 'encrypted':
                    ...
                    try:
                        ...
                        print(f"✅ Successfully recovered aposematic image: {decoded_path}")
                    except Exception as e:
                    ...
                    try:
                        ...
                        if serialized_token:
                            ...
                            if valid and audio_base64:
                                item['audio'] = {
                                    'data': audio_base64,
                                    'format': detect_audio_format(
                                        __import__('base64').b64decode(audio_base64)
                                    ),
                ...
                try:
                    ...
                    if base64_uri and base64_uri.startswith('data:image'):
                        ...
                        print(f"✅ Successfully converted to base64 URI (length: {len(base64_uri)})")
                    else:
                        print(f"❌ Invalid base64 URI generated: {base64_uri[:100] if base64_uri else 'None'}")
